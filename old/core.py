# core.py - CPU-based fluid solver and renderer (numpy fallback)
#
# This module provides a pure-numpy implementation that can be used
# when a CUDA GPU is not available.  It is NOT imported by the main
# GPU pipeline (engine.py) — it serves as a standalone fallback.
...
# ---------------------------------------------------------------------------
#  Vectorised HSV -> RGB  (operates on full NxN arrays at once)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
#  FluidSolver  (Stam's stable fluids, pure numpy)
# ---------------------------------------------------------------------------
class FluidSolver:
    def __init__(self, N, dt=0.1, diffusion=0.00008, viscosity=0.00003):
        ...
        # Pre-compute advection coordinate grids
        ii, jj = np.meshgrid(
        ...
        # Circular brush mask for add_density / add_velocity
        self._build_brush(radius=5)

    def _build_brush(self, radius=5):
    ...
    def _advect(self, b, d, d0, u, v):
        ...
        if d.ndim == 2:
            ...
            s0e = s0[..., None]; t0e = t0[..., None]
            s1e = s1[..., None]; t1e = t1[..., None]
            d[1:-1, 1:-1] = (
                s0e * (t0e * d0[i0, j0] + t1e * d0[i0, j1])
                + s1e * (t0e * d0[i1, j0] + t1e * d0[i1, j1])
            )
            self._bnd(0, d)
    ...
    def add_density(self, cx, cy, amount, color):
        ...
        ni, nj, fv = ni[ok], nj[ok], self._sp_f[ok]
...
# ---------------------------------------------------------------------------
#  Renderer  (CPU, numpy -> uint8 image)
# ---------------------------------------------------------------------------
class Renderer:
    def __init__(self, N):
        self.N = N
    ...
    def process(self, density, u, v, t, psyche, mode):
        ...
        # FIX: Original had "Raw Density" mode not listed in RENDER_MODES config.
        # Now only handles the three modes defined in config.py.
